...
|-----------|------------|-------------------|
| **Dev 1** | [SPRINT1_DEV1_FIREBASE_LISTINGS.md](./SPRINT1_DEV1_FIREBASE_LISTINGS.md) | Firestore listings CRUD, listing rules, offer listing photos (Storage) |
...
| **Dev 3** | [SPRINT1_DEV3_PROFILE_AUTH.md](./SPRINT1_DEV3_PROFILE_AUTH.md) | Profile/settings, Google login, change + forgot password, follow, **Firebase Hosting**, final review passes |
...
5. **Firebase Storage:** **Listing** photos (optional, offers only) — see Dev 1 sprint doc. **Profile** photos use URLs/initials/Google photo — **no** profile Storage ([PRODUCT_DECISIONS.md](./PRODUCT_DECISIONS.md)).
...
- [PRODUCT_DECISIONS.md](./PRODUCT_DECISIONS.md) — **finalized product choices** (avatar, follow, hosting, password)
...
| `src/services/authService.ts` | `getCurrentUser()` | `getCurrentUser()` | Google login, change + forgot password |
...
- Button + wording **final review** (Dev 3).
- **Firebase Hosting** setup and deploy (**Dev 3** — see Dev 3 sprint doc).
...
- [ ] **Email/password users** can **change password** and use **forgot password**; Google-only users see **“Password is managed by Google.”** (Dev 3).
